...
####Generate conversion table for Hong Kong
#3' 5' base pair region obtained from:
#PB2 #https://www.ncbi.nlm.nih.gov/nuccore/KY321924
#PB1 #https://www.ncbi.nlm.nih.gov/nuccore/KY321925
#PA #https://www.ncbi.nlm.nih.gov/nuccore/KY321926
#HA #https://www.ncbi.nlm.nih.gov/nuccore/KY321927
#NP #https://www.ncbi.nlm.nih.gov/nuccore/KY321928
#NA #https://www.ncbi.nlm.nih.gov/nuccore/KY321929
#M #https://www.ncbi.nlm.nih.gov/nuccore/KY321930
#NS #https://www.ncbi.nlm.nih.gov/nuccore/KY321931
templates <- c('UCCCA','UCCAU','UCCUC','UCCCA','UCCCC','UCCAU','UCCGU','UCCAG') #3' vRNA
additions <- c('AGGGU','AGGUA','AGGAG','AGGGU','AGGGU','AGGUA','AGGCA','AGGUC') #5' vRNA

templates <- substr(templates, 1, 5)
additions <- substr(additions, 1, 5)
...
nrgConverter$Subunit = c('NS','M','NA','NP','HA','PA','PB1','PB2')
nrgConverter <- nrgConverter[,c(4,1,2,3)]
#setwd() +#if not using the default directory
setwd("./Helical Stability/")
write.csv(nrgConverter, '3 prime 5 prime bonds.csv', row.names = F)
...
df <- read.csv(paste(strains[2], ' strain.csv', sep=''), row.names = 1, stringsAsFactors = F)[1:8, c(1,6)]
nrgConverter <- nrgConverter[order(nrgConverter$Subunit, c(gsub('\\.', '', df$x1))),]
df <- data.frame(Subunit = nrgConverter$Subunit, nrg1 = nrgConverter$TotalEnergy, PAR = df$PAR, seq3 = nrgConverter$Templates, seq5 = nrgConverter$Additions, stringsAsFactors = F)

rm(nrgConverter)

#get values for 3'C4 and 3'U4
templates <- c('CGCU','CGUU')
additions <- c('GCG','GCA')
...
#add values to df
df$nrg2 <- 0
df$nrg2[C] <- nrgConverter[1,3]
df$nrg2[U] <- nrgConverter[2,3] 
df$nrgDif <- df$nrg1 - df$nrg2
df$nrgRatio <- df$nrg1 / df$nrg2

print('Energy to PAR')
x <- summary(lm(data=df, nrg1 ~ PAR))
x
x$r.squared

print('Energy Difference to PAR')
y <- summary(lm(data=df, nrgDif ~ PAR))
y
y$r.squared

print('Energy Ratio to PAR')
z <- summary(lm(data=df, nrgRatio ~ PAR))
z
z$r.squared

colnames(df) <- c('Segment', "3' 5' Energy", 'PAR', "3' Sequence", "5' Sequence", "Energy 3'C2 to 3'Y4", 'Difference', 'Ratio')
df <- df[,c(1,3,4,5,2,6,7,8)]
write.csv(file="Hong Kong 3' 5' base pair region and PAR.csv", df, row.names = F)
